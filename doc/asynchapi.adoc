...
This is the callback that will be executed,  as *func(transfer, status)*, when the
transfer either completes, fails, or is canceled. The _transfer_ object is automatically
deleted as soon as the callback returns.
